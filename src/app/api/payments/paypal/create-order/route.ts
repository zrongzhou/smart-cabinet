 ...
 * Create a PayPal order for an existing order and return the approval URL.
 */
export async function POST(request: NextRequest) {
  try {
    ...
    const { orderId, amount, currency, locale } = body;
    ...
    const result = await provider.createPayment({
      ...
      currency: currency || getCurrencyForMethod('paypal'),
      method: 'paypal',
      locale,
    });
