...
export default function AdminLayout({ children }: AdminLayoutProps) {
  ...
  useEffect(() => {
    if (!mounted) return;
    if (!isAuthenticated && pathname !== '/xiaozhouBackend/login') {
  ...
  const navLinks = [
    { href: '/xiaozhouBackend', label: '仪表盘', icon: LayoutDashboard },
    { href: '/xiaozhouBackend/products', label: '产品管理', icon: Package },
    { href: '/xiaozhouBackend/categories', label: '分类/标签', icon: Tag },
    { href: '/xiaozhouBackend/blog', label: '博客管理', icon: FileText },
    { href: '/xiaozhouBackend/faqs', label: 'FAQ管理', icon: HelpCircle },
    { href: '/xiaozhouBackend/media', label: '媒体库', icon: ImageIcon },
    { href: '/xiaozhouBackend/editor', label: '页面编辑', icon: Edit3 },
    { href: '/xiaozhouBackend/settings', label: '站点设置', icon: Settings },
    { href: '/xiaozhouBackend/contact-messages', label: '联系消息', icon: MessageSquare },
  ];
      ...
      <div className="min-h-screen flex flex-col bg-slate-50">
        ...
        <header className="sticky top-0 z-40 h-16 bg-white/90 backdrop-blur-xl border-b border-slate-200/80 shadow-sm flex items-center justify-between px-4 sm:px-6 gap-3">
          <div className="flex items-center gap-3 min-w-0">
            <Link href="/xiaozhouBackend" className="flex items-center gap-2 shrink-0">
